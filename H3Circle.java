...
public class H3Circle {
	...
	public void drawGeometry(GraphicsContext3D gc, double radius, double x, double y) {
		establishTransform(gc, radius, x, y);
		gc.draw(m_lines);
	}
	...
	private void establishTransform(GraphicsContext3D gc, double radius, double x, double y) {
	...
	private void createCircle(double radius, int numSegments) {
		...
		ColoringAttributes coloringAttributes = new ColoringAttributes(0.9f, 0.5f, 0.0f, ColoringAttributes.FASTEST);
		...
		Point3f[] xyPoints = createXYCircleCoordinates((float) radius, numSegments);
