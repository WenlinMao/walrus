...
public class H3AdaptiveRenderLoop implements H3RenderLoop, Runnable {
	////////////////////////////////////////////////////////////////////////
	// CONSTRUCTORS
	////////////////////////////////////////////////////////////////////////

	public H3AdaptiveRenderLoop(H3Graph graph, H3Canvas3D canvas, H3ViewParameters parameters,
			H3Transformer transformer, H3RenderQueue queue, H3AdaptiveRenderer renderer) {
		m_graph = graph;
		m_canvas = canvas;
	...
	public synchronized void resizeDisplay() {
		...
		{
			if (DEBUG_PRINT) {
				System.out.println("resizing display (" + STATE_NAMES[m_state] + ")");
			}
			m_picker.reset();
		}
		endRequest();
	...
	public synchronized void rotateDisplay(H3RotationRequest request) {
		...
		{
			if (DEBUG_PRINT) {
				System.out.println("rotating display (" + STATE_NAMES[m_state] + ")");
			}
	...
	public synchronized H3DisplayPosition getDisplayPosition() {
		...
		startRequest();
		{
			...
			retval = new H3DisplayPosition(position.startingNode, m_parameters.getObjectTransform(),
					position.transform);
		}
		endRequest();
		return retval;
		...
		while (m_state == STATE_IDLE) {
			...
			if (m_restoreDisplayRequested) {
				...
				if (m_displayPosition == null) {
					...
					m_parameters.setObjectTransform(m_displayPosition.getRotation());

					H3Transformer.Position position = new H3Transformer.Position();
	...
	private void beTranslateState() {
		...
		while (more) {
			...
			if (sourceLength > TRANSLATION_THRESHOLD) {
				double delta = Math.max(TRANSLATION_MIN_DELTA, 1.0 - sourceLength);
				double scale = sourceLength / (sourceLength - delta);

				destination.set(source);
				destination.w *= scale;
			} else if (sourceLength > TRANSLATION_STEP_DISTANCE) {
				double scale = sourceLength / (sourceLength - TRANSLATION_STEP_DISTANCE);
	...
	private synchronized boolean synchCompleteState() {
		if (m_numPendingRequests > 0) {
			if (DEBUG_PRINT) {
				...
				System.out.println("(" + m_numPendingRequests + " request(s) pending)");
			}
	...
	private void translate(Point4d source, Point4d destination) {
		...
		Matrix4d translation = H3Transform.buildTranslation(source, destination);
	...
	private static final String[] STATE_NAMES = { "STATE_SHUTDOWN", "STATE_IDLE", "STATE_ROTATE", "STATE_TRANSLATE",
	...
	private static final double TRANSLATION_THRESHOLD = 1.0 - TRANSLATION_STEP_DISTANCE;
