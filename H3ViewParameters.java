...
public class H3ViewParameters {
    ...
    public H3ViewParameters(H3Canvas3D canvas) {
        ...
        m_pickEquivalenceRadius = PICK_EQUIVALENCE_RADIUS_PIXELS * m_pixelToMeterScale;
        ...
        m_depthCueing.setInfluencingBounds(new BoundingSphere(new Point3d(0.0, 0.0, 0.0), 100.0));
    }
    ...
    public void drawLabel(GraphicsContext3D gc, double x, double y, int zOffset, String s) {
        ...
        Point3d eye = getEye();
        double labelZ = zOffset * LABEL_Z_OFFSET_SCALE + LABEL_Z_SCALE * eye.z + LABEL_Z_OFFSET;
        ...
        Text2D text = new Text2D(s, new Color3f(1.0f, 1.0f, 1.0f), "Courier", 24, Font.BOLD);
    ...
    private void updateDepthCueing() {
        if (m_depthCueingEnabled) {
            double front = DEPTH_CUEING_ENABLED_FRONT * Math.pow(m_magnification, -1.2);
            double back = DEPTH_CUEING_ENABLED_BACK * Math.pow(m_magnification, -0.7);
    ...
    private double computePixelToMeterScale(Screen3D screen) {
        ...
        if (Math.abs(xScale - yScale) > 1.0e-10) {
            System.err.println(
                    "WARNING: computePixelToMeterScale(): " + "xScale(" + xScale + ") != yScale(" + yScale + ").");
        }
    ...
    private void initializeBasicAppearances() {
        ...
        ColoringAttributes pointColoringAttributes = new ColoringAttributes(0.0f, 0.0f, 1.0f,
                ColoringAttributes.FASTEST);

        ColoringAttributes lineColoringAttributes = new ColoringAttributes(0.7f, 0.7f, 0.7f,
    ...
    private void initializeGraphAppearances() {
        ...
        ColoringAttributes coloringAttributes = new ColoringAttributes(1.0f, 1.0f, 0.0f, ColoringAttributes.FASTEST);

        ColoringAttributes treeColoringAttributes = new ColoringAttributes(0.11765f, 0.58824f, 0.1f,
                ColoringAttributes.FASTEST);

        ColoringAttributes nontreeColoringAttributes = new ColoringAttributes(0.7f, 0.7f, 0.7f,
                ColoringAttributes.FASTEST);
        ...
        m_nontreeLinkAppearance.setColoringAttributes(nontreeColoringAttributes);
    }
    ...
    private void initializePickAppearances() {
        ...
        ColoringAttributes pointColoringAttributes = new ColoringAttributes(0.9f, 0.1f, 0.1f,
                ColoringAttributes.FASTEST);
