...
public class H3Main {
	...
	private void handleOpenFileRequest() {
		...
		if (file != null) { 
			...
			try {
				ASCIIInputStreamReader reader = new ASCIIInputStreamReader(new FileInputStream(file));
				...
				dialog.showMessageDialog(null, msg, "File Not Found", JOptionPane.ERROR_MESSAGE);
			}
	...
	private RenderingConfiguration createRenderingConfigurationSnapshot() {
		...
		retval.automaticExtendedPrecision = m_automaticExtendedPrecisionMenuItem.isSelected();
		retval.nodeColor = m_colorSchemeMenu.createNodeColorConfigurationSnapshot();
		retval.treeLinkColor = m_colorSchemeMenu.createTreeLinkColorConfigurationSnapshot();
		retval.nontreeLinkColor = m_colorSchemeMenu.createNontreeLinkColorConfigurationSnapshot();
		...
		for (int i = 0; i < numAttributes; i++) {
			...
			if (menuItem != null && menuItem.isSelected()) {
				...
				AttributeDefinitionIterator iterator = m_backingGraph.getAttributeDefinition(name);
				if (iterator.atEnd()) {
	...
	private void populateMenus(Graph graph) {
		m_spanningTreeQualifiers = m_graphLoader.loadSpanningTreeQualifiers(graph);
		{
		...
		m_nodeLabelAttributes = m_graphLoader.loadAttributes(graph, m_allAttributeTypeMatcher);
		{
	...
	private void handleStartRenderingRequest() {
		RenderingConfiguration renderingConfiguration = createRenderingConfigurationSnapshot();
	...
	private void colorNodes(ColorConfiguration configuration) {
		...
		// be sure.
		setNodeTransparencyEnabled(configuration.isTransparent && configuration.scheme != ColorConfiguration.RGB);
		...
		case ColorConfiguration.FIXED_COLOR: {
			ColoringAttributes attributes = makeColoringAttributes(configuration.fixedColor);

			m_viewParameters.getNodeAppearance().setColoringAttributes(attributes);
			m_viewParameters.getNearNodeAppearance().setColoringAttributes(attributes);
			m_viewParameters.getMiddleNodeAppearance().setColoringAttributes(attributes);
			m_viewParameters.getFarNodeAppearance().setColoringAttributes(attributes);
		}
			break;
	...
	private void setNodeTransparencyEnabled(boolean state) {
		TransparencyAttributes attributes = (state ? m_viewParameters.getTransparencyAttributes() : null);

		m_viewParameters.getNodeAppearance().setTransparencyAttributes(attributes);
		m_viewParameters.getNearNodeAppearance().setTransparencyAttributes(attributes);
		m_viewParameters.getMiddleNodeAppearance().setTransparencyAttributes(attributes);
		m_viewParameters.getFarNodeAppearance().setTransparencyAttributes(attributes);
	}
	...
	private void colorTreeLinks(ColorConfiguration configuration) {
		...
		// draw with transparency, and furthermore, it occasionally
		// draws lines (and perhaps points) in black.
		setTreeLinkTransparencyEnabled(configuration.isTransparent && configuration.scheme != ColorConfiguration.RGB);
		...
		case ColorConfiguration.FIXED_COLOR: {
			ColoringAttributes attributes = makeColoringAttributes(configuration.fixedColor);
			m_viewParameters.getTreeLinkAppearance().setColoringAttributes(attributes);
		}
			break;
	...
	private void setTreeLinkTransparencyEnabled(boolean state) {
		...
		m_viewParameters.getTreeLinkAppearance().setTransparencyAttributes(attributes);
	}
	...
	private void colorNontreeLinks(ColorConfiguration configuration) {
		...
		setNontreeLinkTransparencyEnabled(
				configuration.isTransparent && configuration.scheme != ColorConfiguration.RGB);
		...
		case ColorConfiguration.FIXED_COLOR: {
			ColoringAttributes attributes = makeColoringAttributes(configuration.fixedColor);
			m_viewParameters.getNontreeLinkAppearance().setColoringAttributes(attributes);
		}
			break;
	...
	private void setNontreeLinkTransparencyEnabled(boolean state) {
		...
		m_viewParameters.getNontreeLinkAppearance().setTransparencyAttributes(attributes);
	}
	...
	private void colorNodesRGB(String colorAttribute) {
		int attribute = m_backingGraph.getAttributeDefinition(colorAttribute).getID();
		...
		for (int i = 0; i < numNodes; i++) {
			...
			try {
				...
				ValueIterator iterator = m_backingGraph.getNodeAttribute(nodeID, attribute);
		...
		for (int i = 0; i < numLinks; i++) {
			if (m_graph.checkTreeLink(i) == treeLink) {
				...
				try {
					...
					ValueIterator iterator = m_backingGraph.getLinkAttribute(linkID, attribute);
					color = extractRGBColor(iterator);
				} catch (AttributeUnavailableException e) {
					// Nothing to do--simply use the default color.
				}
	...
	private void selectNodes(ColorConfiguration configuration) {
		if (configuration.selectionAttribute == null) {
			...
			int attribute = m_backingGraph.getAttributeDefinition(configuration.selectionAttribute).getID();
			...
			for (int i = 0; i < numNodes; i++) {
				...
				try {
					int nodeID = m_graph.getNodeID(i);
					isSelected = m_backingGraph.getBooleanAttribute(ObjectType.NODE, nodeID, attribute);
				} catch (AttributeUnavailableException e) {
					// Assume selected.
				}
	...
	private void selectLinks(ColorConfiguration configuration, boolean treeLink) {
		if (configuration.selectionAttribute == null) {
			...
			for (int i = 0; i < numLinks; i++) {
				if (m_graph.checkTreeLink(i) == treeLink) {
					...
					try {
						int linkID = m_graph.getLinkID(i);
						isSelected = m_backingGraph.getBooleanAttribute(ObjectType.LINK, linkID, attribute);
					} catch (AttributeUnavailableException e) {
						// Assume selected.
					}
	...
	private boolean setupRendering(RenderingConfiguration renderingConfiguration) {
		boolean retval = true;
		try {
			if (m_renderingConfiguration == null
					|| !renderingConfiguration.spanningTree.equals(m_renderingConfiguration.spanningTree)) {
				m_displayPosition = null;
				m_savedDisplayPosition = null;
				m_isDisplayNarrowed = false;

				m_graph = m_graphLoader.load(m_backingGraph, renderingConfiguration.spanningTree);
				...
				if (numReachable == numNodes) {
					...
					if (numReachable == -1) {
						...
						msg = "The selected spanning tree does not cover" + " all nodes.\nOnly " + numReachable + " of "
								+ numNodes + " nodes are reachable through the" + " spanning tree.";
					}

					JOptionPane.showMessageDialog(null, msg, "Malformed Spanning Tree", JOptionPane.ERROR_MESSAGE);
				}
			} else {
				boolean visibilityChanged = false;

				if (!renderingConfiguration.nodeColor.equalColoring(m_renderingConfiguration.nodeColor)) {
					visibilityChanged = true;
					colorNodes(renderingConfiguration.nodeColor);
					selectNodes(renderingConfiguration.nodeColor);
				}

				if (!renderingConfiguration.treeLinkColor.equalColoring(m_renderingConfiguration.treeLinkColor)) {
					visibilityChanged = true;
					colorTreeLinks(renderingConfiguration.treeLinkColor);
					selectLinks(renderingConfiguration.treeLinkColor, true);
				}

				if (!renderingConfiguration.nontreeLinkColor.equalColoring(m_renderingConfiguration.nontreeLinkColor)) {
					visibilityChanged = true;
					colorNontreeLinks(renderingConfiguration.nontreeLinkColor);
					selectLinks(renderingConfiguration.nontreeLinkColor, false);
				}
			...
			JOptionPane dialog = new JOptionPane();
			dialog.showMessageDialog(null, msg, "Rendering Setup Failed", JOptionPane.ERROR_MESSAGE);
		}
		return retval;
	}

	private boolean layoutGraph(RenderingConfiguration renderingConfiguration, boolean useExtendedPrecision) {
		H3GraphLayout layout = new H3GraphLayout(renderingConfiguration.automaticExtendedPrecision);

		H3GraphLayout.LayoutState layoutState = layout.layoutHyperbolic(m_graph, useExtendedPrecision);
		...
		if (!retval) {
			...
			if (useExtendedPrecision) {
				...
				JOptionPane dialog = new JOptionPane();
				int response = dialog.showConfirmDialog(null, msg, "Graph Layout Failed", JOptionPane.YES_NO_OPTION);
				retval = (response == JOptionPane.YES_OPTION);
				if (retval) {
					m_graph.sanitizeLayoutCoordinates();
				}
			} else {
				String msg = "Graph could not be laid out within the" + " limits of floating-point precision.\n"
						+ "Layout failed for " + numBadNodes + " of " + numNodes + " nodes.\n\n"
						+ "You may retry using extended precision arithmetic,"
						+ " but layout can take several hours for large graphs.\n"
						+ "Or you may proceed by arbitrarily placing the" + " problematic nodes at the origin.\n"
						+ "Or you may cancel the layout attempt and not render" + " the graph.\n";
				JOptionPane dialog = new JOptionPane();
				int response = dialog.showOptionDialog(null, msg, "Graph Layout Failed",
						JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null,
						new String[] { "Retry with Extended Precision", "Place Nodes at Origin", "Cancel" }, null);
	...
	private void checkGraphIDMappings(H3Graph graph, Graph backingGraph) {
		...
		for (int i = 0; i < numLinks; i++) {
			...
			if (iterator.getSource() != sourceID) {
				String msg = "source[" + iterator.getSource() + "] of link[" + linkID + ", " + i + "] != [" + sourceID
						+ "]";
				throw new RuntimeException(msg);
			}

			if (iterator.getDestination() != destinationID) {
				String msg = "destination[" + iterator.getDestination() + "] of link[" + linkID + ", " + i + "] != ["
						+ destinationID + "]";
				throw new RuntimeException(msg);
			}
		}
	...
	private void startRendering(RenderingConfiguration renderingConfiguration) {
		...
		m_viewParameters.setDepthCueingEnabled(renderingConfiguration.depthCueing);
		m_viewParameters.setAxesEnabled(renderingConfiguration.axes);

		boolean useNodeSizes = renderingConfiguration.multipleNodeSizes;
		boolean includeNodes = determineWhetherToIncludeObject(renderingConfiguration.nodeColor);
		boolean includeTreeLinks = determineWhetherToIncludeObject(renderingConfiguration.treeLinkColor);
		boolean includeNontreeLinks = determineWhetherToIncludeObject(renderingConfiguration.nontreeLinkColor);
		boolean includeNodeColor = determineWhetherToIncludeColor(renderingConfiguration.nodeColor);
		boolean includeTreeLinkColor = determineWhetherToIncludeColor(renderingConfiguration.treeLinkColor);
		boolean includeNontreeLinkColor = determineWhetherToIncludeColor(renderingConfiguration.nontreeLinkColor);

		H3PointRenderList renderList = new H3PointRenderList(m_graph, useNodeSizes, includeNodes, includeNodeColor,
				includeTreeLinks, includeTreeLinkColor, includeNontreeLinks, includeNontreeLinkColor);

		renderList.setNearNodeAppearance(
				useNodeSizes ? m_viewParameters.getNearNodeAppearance() : m_viewParameters.getMiddleNodeAppearance());
		renderList.setMiddleNodeAppearance(m_viewParameters.getMiddleNodeAppearance());
		renderList.setFarNodeAppearance(m_viewParameters.getFarNodeAppearance());
		renderList.setTreeLinkAppearance(m_viewParameters.getTreeLinkAppearance());
		renderList.setNontreeLinkAppearance(m_viewParameters.getNontreeLinkAppearance());
		...
		if (renderingConfiguration.adaptiveRendering) {
			...
			boolean processNontreeLinks = (renderingConfiguration.nontreeLinkColor.scheme != ColorConfiguration.INVISIBLE);

			H3Transformer transformer = new H3Transformer(m_graph, queue, processNontreeLinks);
			...
			if (true) {
				...
				renderer = new H3CircleRenderer(m_graph, m_viewParameters, queue, renderList);
			}

			// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

			H3AdaptiveRenderLoop adaptive = new H3AdaptiveRenderLoop(m_graph, m_canvas, m_viewParameters, transformer,
					queue, renderer);
			...
			H3NonadaptiveRenderLoop nonadaptive = new H3NonadaptiveRenderLoop(m_graph, m_canvas, m_viewParameters,
		...
		m_eventHandler = new EventHandler(m_viewParameters, m_canvas, m_renderLoop, narrowingHandler, m_rootNode,
				m_currentNode, m_previousNode, m_graph, m_backingGraph, renderingConfiguration.nodeLabelAttributes,
				renderingConfiguration.nodeLabelAttributeNames, m_statusBar, renderingConfiguration.onScreenLabels,
	...
	private boolean determineWhetherToIncludeColor(ColorConfiguration configuration) {
		return (configuration.scheme != ColorConfiguration.INVISIBLE
				&& configuration.scheme != ColorConfiguration.FIXED_COLOR);
	}

	private boolean determineWhetherToIncludeObject(ColorConfiguration configuration) {
		return (configuration.scheme != ColorConfiguration.INVISIBLE);
	}
	...
	private Graph loadGraph(File file, Reader reader) {
		...
		try {
			...
			String msg = "Error parsing file `" + file.getPath() + "': " + e.toString();
			JOptionPane dialog = new JOptionPane();
			dialog.showMessageDialog(null, msg, "Open Failed", JOptionPane.ERROR_MESSAGE);
		}
		...
		if (result == JFileChooser.APPROVE_OPTION) {
			...
			if (!retval.isFile()) {
				if (retval.exists()) {
					...
					JOptionPane dialog = new JOptionPane();
					dialog.showMessageDialog(null, msg, "Invalid Path", JOptionPane.ERROR_MESSAGE);
				} else {
	...
	private void setupIdleRenderingMenu() {
		...
		m_recomputeLayoutExtendedMenuItem.setEnabled(m_renderingConfiguration != null);
	...
	private JMenuBar createInitialMenuBar() {
		...
		m_saveWithLayoutMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S, ActionEvent.CTRL_MASK));
		m_saveWithLayoutMenuItem.setEnabled(false);
		...
		m_closeMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_W, ActionEvent.CTRL_MASK));
		...
		m_recomputeLayoutExtendedMenuItem = new JMenuItem("Recompute Layout With Extended Precision");
		m_recomputeLayoutExtendedMenuItem.setMnemonic(KeyEvent.VK_R);
		m_recomputeLayoutExtendedMenuItem.setEnabled(false);
		m_recomputeLayoutExtendedMenuItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
		...
		m_widenSubtreeMenuItem.setEnabled(false);
		m_widenSubtreeMenuItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int node = m_eventHandler.getCurrentNode();
				handleWidenSubtreeRequest(node);
			}
		...
		m_widenTowardRootMenuItem.setEnabled(false);
		m_widenTowardRootMenuItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int node = m_eventHandler.getCurrentNode();
				handleWidenTowardRootRequest(node);
			}
		...
		m_refreshDisplayMenuItem.setEnabled(false);
		m_refreshDisplayMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_R, ActionEvent.CTRL_MASK));
		...
		m_wobbleDisplayMenuItem.setEnabled(false);
		m_wobbleDisplayMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_B, ActionEvent.CTRL_MASK));
	...
	private void addPruneToNeighborhoodSubmenus(JMenu menu) {
		...
		for (int i = 0; i < distances.length; i++) {
			...
			JMenuItem menuItem = new JMenuItem("distance <= " + Integer.toString(distance));
	...
	private H3GraphLoader.AttributeTypeMatcher m_allAttributeTypeMatcher = new AllAttributeTypeMatcher();
	...
	private static class EventHandler implements KeyListener, MouseListener, MouseMotionListener {
		public EventHandler(H3ViewParameters parameters, H3Canvas3D canvas, H3RenderLoop renderLoop,
				NarrowingEventHandler narrowingHandler, int rootNode, int currentNode, int previousNode, H3Graph graph,
				Graph backingGraph, int[] nodeLabelAttributes, String[] nodeLabelAttributeNames, JTextField statusBar,
			...
			m_labelConstructor = new NodeLabelConstructor(backingGraph, nodeLabelAttributes);
		...
		public void forceIdleState() {
			...
			default:
				throw new RuntimeException("Invalid state in EventHandler: invalid state " + m_state);
			}
			m_state = STATE_IDLE;
		}

		public void refreshDisplay() {
			if (m_state == STATE_IDLE || m_state == STATE_ROTATING_INTERACTIVE_START) {
		...
		public void mousePressed(MouseEvent e) {
			...
			case STATE_ROTATING_CONTINUOUS_START:
				...
				break;
			default:
				throw new RuntimeException("Invalid state in EventHandler: mousePressed in state " + m_state);
			}
		}
		...
		public void mouseReleased(MouseEvent e) {
			...
			case STATE_WOBBLING:
				// FALLTHROUGH
			default:
				throw new RuntimeException("Invalid state in EventHandler: mouseReleased in state " + m_state);
			}
		}
		...
		public void mouseDragged(MouseEvent e) {
			...
			case STATE_WOBBLING:
				// FALLTHROUGH
			default:
				throw new RuntimeException("Invalid state in EventHandler: mouseDragged in state " + m_state);
			}
		}
		...
		public void mouseMoved(MouseEvent e) {
			...
			default:
				throw new RuntimeException("Invalid state in EventHandler: mouseMoved in state " + m_state);
			}
		}
			...
			if (e.isControlDown()) {
				...
				if (m_state == STATE_IDLE || m_state == STATE_ROTATING_INTERACTIVE_START) {
					if (c == 'w') {
						...
						int distance = c - '0';
						m_narrowingHandler.pruneToNeighborhood(m_currentNode, distance);
					} else if (c == ',') {
		...
		private double transformDeltas(double x) {
			...
			double t = Math.atan(10.0 * (Math.abs(x) - 0.3)) / ROTATION_SCALE + 0.1987918;
		...
		private void dumpMouseEvent(String name, MouseEvent e) {
			...
			int modifiers = e.getModifiers();
			System.out.print(checkModifiers(modifiers, InputEvent.BUTTON1_MASK) ? "1" : "-");
			System.out.print(checkModifiers(modifiers, InputEvent.BUTTON2_MASK) ? "2" : "-");
			System.out.print(checkModifiers(modifiers, InputEvent.BUTTON3_MASK) ? "3" : "-");

			System.out.print(checkModifiers(modifiers, InputEvent.SHIFT_MASK) ? "S" : "-");
			System.out.print(checkModifiers(modifiers, InputEvent.CTRL_MASK) ? "C" : "-");
		...
		private H3InteractiveRotationRequest m_interactiveRequest = new H3InteractiveRotationRequest();

		private H3RepeatingRotationRequest m_repeatingRequest = new H3RepeatingRotationRequest();

		private H3WobblingRotationRequest m_wobblingRequest = new H3WobblingRotationRequest();
		...
		private ComponentResizeListener m_resizeListener = new ComponentResizeListener();

		////////////////////////////////////////////////////////////////////

		private class PaintObserver implements Observer {
	...
	private static class NodeLabelConstructor {
		public NodeLabelConstructor(Graph backingGraph, int[] nodeLabelAttributes) {
			m_backingGraph = backingGraph;
			m_nodeLabelAttributes = nodeLabelAttributes;
		}
		...
		public String[] extractValues(int node, boolean quoteStrings) {
			...
			for (int i = 0; i < m_nodeLabelAttributes.length; i++) {
				...
				try {
					...
					ValueIterator iterator = m_backingGraph.getNodeAttribute(node, attribute);
		...
		private void addAttributeValue(StringBuffer buffer, ValueIterator iterator, boolean quoteStrings) {
			...
			case ValueType._ENUMERATION: {
				...
				ReadOnlyEnumeratorIterator enumerator = m_backingGraph.getEnumerator(value);
	...
	private static class ColorSchemeMenu {
		...
		public ColorSchemeMenu() {
			...
			Map<String, JMenuItem> nodeMenuMap = new HashMap<String, JMenuItem>();
			m_nodeColorMenu = new JMenu("Node Color");
			m_nodeColorMenu.setMnemonic(KeyEvent.VK_N);
			m_nodeColorSelection = new ColorSelection(m_nodeColorMenu, nodeMenuMap, m_fixedColors);

			Map<String, JMenuItem> treeLinkMenuMap = new HashMap<String, JMenuItem>();
			m_treeLinkColorMenu = new JMenu("Tree Link Color");
			m_treeLinkColorMenu.setMnemonic(KeyEvent.VK_T);
			m_treeLinkColorSelection = new ColorSelection(m_treeLinkColorMenu, treeLinkMenuMap, m_fixedColors);

			Map<String, JMenuItem> nontreeLinkMenuMap = new HashMap<String, JMenuItem>();
			m_nontreeLinkColorMenu = new JMenu("Nontree Link Color");
			m_nontreeLinkColorMenu.setMnemonic(KeyEvent.VK_E);
			m_nontreeLinkColorSelection = new ColorSelection(m_nontreeLinkColorMenu, nontreeLinkMenuMap, m_fixedColors);

			createPredefinedColorSchemes(nodeMenuMap, treeLinkMenuMap, nontreeLinkMenuMap);
			{
				m_predefinedColorSchemeMenu = new JMenu("Predefined");
				m_predefinedColorSchemeMenu.setMnemonic(KeyEvent.VK_P);
				ListIterator iterator = m_predefinedColorSchemes.listIterator();
				while (iterator.hasNext()) {
					PredefinedColorScheme scheme = (PredefinedColorScheme) iterator.next();
		...
		public void populateAttributeMenus(H3GraphLoader graphLoader, Graph graph) {
			...
			m_nontreeLinkColorSelection.populateAttributeMenus(m_attributeCache);
		}
		...
		public void enableDefaultColorScheme() {
			if (m_predefinedColorSchemes.size() > 0) {
				...
				PredefinedColorScheme scheme = (PredefinedColorScheme) m_predefinedColorSchemes.get(0);
				scheme.activate();
			}
		}
		...
		public ColorConfiguration createNontreeLinkColorConfigurationSnapshot() {
			return m_nontreeLinkColorSelection.createColorConfigurationSnapshot();
		}
		...
		private void createPredefinedColorSchemes(Map<String, JMenuItem> nodeMenuMap,
				Map<String, JMenuItem> treeLinkMenuMap, Map<String, JMenuItem> nontreeLinkMenuMap) {
			ColorSchemeMaker maker = new ColorSchemeMaker(nodeMenuMap, treeLinkMenuMap, nontreeLinkMenuMap);
			m_predefinedColorSchemes = new ArrayList<PredefinedColorScheme>();
			m_predefinedColorSchemes
					.add(maker.make("Yellow-Green-[Grey]", "Yellow", ColorSchemeMaker.DISABLE_TRANSPARENCY, "Green",
							ColorSchemeMaker.DISABLE_TRANSPARENCY, "Grey", ColorSchemeMaker.ENABLE_TRANSPARENCY));
			m_predefinedColorSchemes
					.add(maker.make("Green-Olive-[Summer Sky]", "Green", ColorSchemeMaker.DISABLE_TRANSPARENCY, "Olive",
							ColorSchemeMaker.DISABLE_TRANSPARENCY, "Summer Sky", ColorSchemeMaker.ENABLE_TRANSPARENCY));
			m_predefinedColorSchemes.add(maker.make("Invisible-Olive-[Summer Sky]", ColorSelection.INVISIBLE,
					ColorSchemeMaker.IGNORE_TRANSPARENCY, "Olive", ColorSchemeMaker.DISABLE_TRANSPARENCY, "Summer Sky",
					ColorSchemeMaker.ENABLE_TRANSPARENCY));
			m_predefinedColorSchemes
					.add(maker.make("Beige-Gold-[Green]", "Beige", ColorSchemeMaker.DISABLE_TRANSPARENCY, "Gold",
							ColorSchemeMaker.DISABLE_TRANSPARENCY, "Green", ColorSchemeMaker.ENABLE_TRANSPARENCY));
		}

		private void createFixedColors() {
			m_fixedColors = new ArrayList<FixedColor>();
			m_fixedColors.add(new FixedColor("Magenta", packRGB(199, 21, 133)));
			m_fixedColors.add(new FixedColor("Red", packRGB(255, 0, 0)));
			m_fixedColors.add(new FixedColor("Pink", packRGB(255, 48, 48)));
			m_fixedColors.add(new FixedColor("Orange", packRGB(255, 140, 0)));
			m_fixedColors.add(new FixedColor("Brown", packRGB(205, 127, 50)));
			m_fixedColors.add(new FixedColor("Gold", packRGB(255, 215, 0)));
			m_fixedColors.add(new FixedColor("Yellow", packRGB(255, 255, 0)));
			m_fixedColors.add(new FixedColor("Beige", packRGB(255, 255, 160)));
			m_fixedColors.add(new FixedColor("Olive", packRGB(202, 255, 112)));
			m_fixedColors.add(new FixedColor("Green", packRGB(30, 150, 25)));
			m_fixedColors.add(new FixedColor("Aquamarine", packRGB(112, 219, 147)));
			m_fixedColors.add(new FixedColor("Summer Sky", packRGB(56, 176, 222)));
			m_fixedColors.add(new FixedColor("Turquoise", packRGB(173, 234, 234)));
			m_fixedColors.add(new FixedColor("Grey", packRGB(178, 178, 178)));
			m_fixedColors.add(new FixedColor("White", packRGB(255, 225, 255)));
		}
		...
		private class PredefinedColorScheme implements ActionListener {
			public PredefinedColorScheme(String name, JMenuItem nodeMenuItem, JMenuItem nodeTransparencyMenuItem,
					boolean nodeTransparency, JMenuItem treeLinkMenuItem, JMenuItem treeLinkTransparencyMenuItem,
					boolean treeLinkTransparency, JMenuItem nontreeLinkMenuItem,
					JMenuItem nontreeLinkTransparencyMenuItem, boolean nontreeLinkTransparency) {
				this.name = name;
				...
				this.treeLinkTransparencyMenuItem = treeLinkTransparencyMenuItem;
				this.treeLinkTransparency = treeLinkTransparency;
				this.nontreeLinkMenuItem = nontreeLinkMenuItem;
				this.nontreeLinkTransparencyMenuItem = nontreeLinkTransparencyMenuItem;
			...
			public void activate() {
				...
				if (treeLinkTransparencyMenuItem != null) {
					treeLinkTransparencyMenuItem.setSelected(treeLinkTransparency);
				}

				nontreeLinkMenuItem.doClick();
				if (nontreeLinkTransparencyMenuItem != null) {
					nontreeLinkTransparencyMenuItem.setSelected(nontreeLinkTransparency);
				}
			}
		...
		private class ColorSchemeMaker {
			...
			public ColorSchemeMaker(Map<String, JMenuItem> nodeMenuMap, Map<String, JMenuItem> treeLinkMenuMap,
				...
				m_nodeTransparencyMenuItem = findColor(nodeMenuMap, ColorSelection.TRANSPARENT);
				m_treeLinkTransparencyMenuItem = findColor(treeLinkMenuMap, ColorSelection.TRANSPARENT);
				m_nontreeLinkTransparencyMenuItem = findColor(nontreeLinkMenuMap, ColorSelection.TRANSPARENT);
			}

			public PredefinedColorScheme make(String name, String nodeColor, int nodeTransparencyType,
					String treeLinkColor, int treeLinkTransparencyType, String nontreeLinkColor,
					int nontreeLinkTransparencyType) {
				JMenuItem nodeMenuItem = findColor(m_nodeMenuMap, nodeColor);
				JMenuItem treeLinkMenuItem = findColor(m_treeLinkMenuMap, treeLinkColor);
				JMenuItem nontreeLinkMenuItem = findColor(m_nontreeLinkMenuMap, nontreeLinkColor);

				JMenuItem nodeTransparencyMenuItem = (nodeTransparencyType == IGNORE_TRANSPARENCY ? null
						: m_nodeTransparencyMenuItem);
				JMenuItem treeLinkTransparencyMenuItem = (treeLinkTransparencyType == IGNORE_TRANSPARENCY ? null
						: m_treeLinkTransparencyMenuItem);
				JMenuItem nontreeLinkTransparencyMenuItem = (nontreeLinkTransparencyType == IGNORE_TRANSPARENCY ? null
						: m_nontreeLinkTransparencyMenuItem);

				boolean nodeTransparency = (nodeTransparencyType == ENABLE_TRANSPARENCY);
				boolean treeLinkTransparency = (treeLinkTransparencyType == ENABLE_TRANSPARENCY);
				boolean nontreeLinkTransparency = (nontreeLinkTransparencyType == ENABLE_TRANSPARENCY);

				return new PredefinedColorScheme(name, nodeMenuItem, nodeTransparencyMenuItem, nodeTransparency,
						treeLinkMenuItem, treeLinkTransparencyMenuItem, treeLinkTransparency, nontreeLinkMenuItem,
						nontreeLinkTransparencyMenuItem, nontreeLinkTransparency);
			}

			private JMenuItem findColor(Map map, String color) {
				JMenuItem retval = (JMenuItem) map.get(color);
				if (retval == null) {
					String msg = "INTERNAL ERROR: color[" + color + "] not found";
					throw new RuntimeException(msg);
				}
				return retval;
			}
	...
	private static class ColorSelection {
		...
		public ColorSelection(JMenu menu, Map<String, JMenuItem> map, List fixedColors) {
			...
			menu.add(m_invisibleMenuItem);
			{
				...
				m_fixedColorMenuItems = new JRadioButtonMenuItem[numFixedColors];

				int index = 0;
				ListIterator iterator = fixedColors.listIterator();
				while (iterator.hasNext()) {
					FixedColor fixedColor = (FixedColor) iterator.next();
					JRadioButtonMenuItem menuItem = new JRadioButtonMenuItem(fixedColor.name);
		...
		public void populateAttributeMenus(AttributeCache attributeCache) {
			...
			m_scalarColorAttributeMenus = createColorAttributeMenuCache(attributeCache.getScalarColorAttributes(),
					m_scalarColorAttributeButtonGroup);

			m_RGBColorAttributeButtonGroup = new ButtonGroup();
			m_RGBColorAttributeMenus = createColorAttributeMenuCache(attributeCache.getRGBColorAttributes(),
					m_RGBColorAttributeButtonGroup);

			populateSelectionAttributeMenu(attributeCache.getSelectionAttributes());
		...
		public ColorConfiguration createColorConfigurationSnapshot() {
			...
			if (m_invisibleMenuItem.isSelected()) {
				...
				retval.colorAttribute = findSelectedMenuItem(m_colorAttributeMenu).getText();
			...
			if (m_selectionAttributeMenu.isEnabled()) {
				JMenuItem menuItem = findSelectedMenuItem(m_selectionAttributeMenu);
		...
		private JRadioButtonMenuItem[] createColorAttributeMenuCache(List attributes, ButtonGroup buttonGroup) {
		...
		private void updateMenuInterdependencies() {
			...
			boolean enableSelection = m_selectionAttributeMenu.getItemCount() > 0 && !m_invisibleMenuItem.isSelected();
			m_selectionAttributeMenu.setEnabled(enableSelection);

			boolean enableTransparency = !m_invisibleMenuItem.isSelected() && !m_hueMenuItem.isSelected()
		...
		/**
		 * Update <code>m_selectedFixedColorIndex</code> if the given selected menu item
		 * is that for a fixed color.
		 *
		 * <p>
		 * Usually, this value is automatically updated by an action listener added to
		 * the menu item representing a fixed color. That works well in general, but
		 * during program initialization, when we want to setup the default color
		 * scheme, we need a different mechanism. Even during initialization, we could
		 * conceivably call <code>JMenuItem.doClick()</code> (which is inherited from
		 * <code>AbstractButton</code>) and simply take advantage of the mechanism
		 * involving action listeners. But despite its attractiveness at first glance,
		 * this approach is not really tenable, owing to the assumptions that an action
		 * listener may make about which parts of the program are fully initialized.
		 * That is to say, though it could work in many cases, it could also be the
		 * source of subtle bugs. So it seems much better to be cautious and have this
		 * other mechanism for keeping <code>m_selectedFixedColorIndex</code> up-to-date
		...
		private void putChecked(Map<String, JMenuItem> map, String name, JMenuItem data) {
			if (map.put(name, data) != null) {
				String msg = "INTERNAL ERROR: mapping already exists for `" + name + "'";
				throw new RuntimeException(msg);
			}
		}
		...
		private class FixedColorListener implements ActionListener {
	...
	private static class RenderingConfiguration {
		...
		public void print() {
			...
			System.out.println("\tautomaticExtendedPrecision = " + automaticExtendedPrecision);
			...
			for (int i = 0; i < nodeLabelAttributes.length; i++) {
				System.out.println("\t" + nodeLabelAttributeNames[i] + " (" + nodeLabelAttributes[i] + ")");
			}
			System.out.println("------------------------------------------\n");
		}
	}
	...
	private static class ColorConfiguration {
		...
		public boolean equalColoring(ColorConfiguration rhs) {
			...
			if (retval) {
				if (scheme == FIXED_COLOR) {
					retval = (rhs.fixedColor == fixedColor && rhs.isTransparent == isTransparent);
			...
			if (retval) {
				...
				if (hasLHS && hasRHS) {
					retval = (rhs.selectionAttribute.equals(selectionAttribute));
				} else {
		...
		private static final String[] m_schemeNames = { "INVISIBLE", "FIXED_COLOR", "HUE", "RGB" };
	}
	...
	private static class AttributeCache {
		public AttributeCache(H3GraphLoader graphLoader, Graph graph) {
			m_scalarColorAttributes = graphLoader.loadAttributes(graph, m_scalarColorAttributeTypeMatcher);
			m_RGBColorAttributes = graphLoader.loadAttributes(graph, m_RGBColorAttributeTypeMatcher);
			m_selectionAttributes = graphLoader.loadAttributes(graph, m_selectionAttributeTypeMatcher);
		}
		...
		private H3GraphLoader.AttributeTypeMatcher m_scalarColorAttributeTypeMatcher = new ScalarColorAttributeTypeMatcher();

		private H3GraphLoader.AttributeTypeMatcher m_RGBColorAttributeTypeMatcher = new RGBColorAttributeTypeMatcher();

		private H3GraphLoader.AttributeTypeMatcher m_selectionAttributeTypeMatcher = new SelectionAttributeTypeMatcher();

		////////////////////////////////////////////////////////////////////

		private class ScalarColorAttributeTypeMatcher implements H3GraphLoader.AttributeTypeMatcher {
			public boolean match(ValueType type) {
				return type == ValueType.INTEGER || type == ValueType.FLOAT || type == ValueType.DOUBLE;
			}
		}

		private class RGBColorAttributeTypeMatcher implements H3GraphLoader.AttributeTypeMatcher {
			public boolean match(ValueType type) {
				return type == ValueType.INTEGER || type == ValueType.FLOAT3 || type == ValueType.DOUBLE3;
			}
		}

		private class SelectionAttributeTypeMatcher implements H3GraphLoader.AttributeTypeMatcher {
			public boolean match(ValueType type) {
	...
	private class AllAttributeTypeMatcher implements H3GraphLoader.AttributeTypeMatcher {
		public boolean match(ValueType type) {
			return true;
		}
	}
	...
	private static class MemoryUsage {
		...
		public void printUsage() {
			...
			System.out.println();
			System.out.println("bufferUsed - baseUsed = " + M(bufferUsed - baseUsed));
			System.out.println("peakUsed - baseUsed = " + M(peakUsed - baseUsed));
			System.out.println("peakUsed - bufferUsed = " + M(peakUsed - bufferUsed));
			System.out.println("finalUsed - baseUsed = " + M(finalUsed - baseUsed));
			System.out.println("finalUsed - peakUsed = " + M(finalUsed - peakUsed));
			System.out.println("===========================================");
		}
