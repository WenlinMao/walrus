...
public class H3Graph {
	...
	public void setNodeLayoutCoordinates(int node, double x, double y, double z, double w) {
	...
	public void computeVisibility() {
		m_nodes.isVisible.conjunction(m_nodes.isDisplayable, m_nodes.isSelected);

		computeNontreeLinkDisplayability();

		m_links.isVisible.conjunction(m_links.isDisplayable, m_links.isSelected);
	}
	...
	// The input node itself is assumed to have been taken care of.
	private void setNeighborhoodDisplayability(int node, int distance, int maxDistance) {
	...
	public int checkTreeReachability(int node) {
		...
		if (numReachable == m_numNodes) {
			String msg = "PASSED: All nodes reachable in tree from node " + node;
			System.out.println(msg);
		} else {
			String msg = "FAILED: Only " + numReachable + " nodes of " + m_numNodes + " reachable in tree from node "
					+ node;
			System.out.println(msg);
	...
	private int checkReachability(BitSet visited, int node) {
		...
		if (visited.get(node)) {
			String msg = "ERROR: Encountered node " + node + " more than once in the spanning tree; there could be"
	...
	public void dumpForTesting2() {
		...
		for (int i = 0; i < m_numNodes; i++) {
			...
			System.out.println("\tparent node: " + (m_nodes.parent[i] >= 0 ? m_links.source[m_nodes.parent[i]] : -1));
			...
			for (int j = treeLinks; j < linksEnd; j++) {
				...
				System.out.println("\t\t" + j + ": " + m_links.source[j] + " => " + m_links.destination[j]);
			}
		}
	}
}
